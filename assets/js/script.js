...
document.addEventListener("DOMContentLoaded", () => {
  const items = document.querySelectorAll(".restoration-faq-item");

  items.forEach(item => {
    gsap.set(item.querySelector(".restoration-faq-content"), {
      height: 0,
      overflow: "hidden"
    });
  });
  ...
  function closeAll() {
    items.forEach(item => {
      ...
      gsap.to(item.querySelector(".restoration-faq-content"), {
        height: 0,
        duration: 0.4,
        ease: "power2.inOut"
      });

      gsap.to(item.querySelector(".restoration-faq-plus"), {
        rotate: 0,
        duration: 0.35,
        ease: "power2.out"
      });
  ...
  items.forEach(item => {
    const trigger = item.querySelector(".restoration-faq-trigger");
    const content = item.querySelector(".restoration-faq-content");
    const icon = item.querySelector(".restoration-faq-plus");
